...
**Version scheme is `<sojourner-semver>+MC<mc-version>`** (e.g. `2.4.2+MC26.3`), tracked in `package.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json` (all three must match). Sojourner versions itself independently — the semver part follows its own release cadence and is never bumped just because a new Minecraft version exists. The `+MC26.3` is semver *build metadata* (not a prerelease tag — deliberately, since a prior `-MC26.3` prerelease-style suffix is what forced Windows onto NSIS-only bundling; WiX/MSI requires a numeric-only prerelease identifier, and build metadata may avoid that restriction, worth re-testing before reverting it): it states the highest MC version Sojourner is verified to support, and only changes as a side effect of actually landing that support (cubiomes updated, a new structure/biome wired up, etc.) — never as a reaction to MC shipping a new version on its own.
