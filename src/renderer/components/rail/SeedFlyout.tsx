...
export default function SeedFlyout() {
  ...
  // "From seed…" reveal in the loaded-seed state (the no-seed state shows
  ...
  const refreshMap = () => dispatch({ type: 'CLEAR_STRUCTURE_CACHE' })
  ...
  const handleManualSeed = () => {
    ...
    try {
      BigInt(trimmed)
      setManualError(null)
      dispatch({ type: 'SET_MANUAL_SEED', seed: trimmed, version: manualVersion, worldType: manualWorldType })
    } catch {
  ...
  const randomizeSeed = () => {
    ...
    setManualSeed(seed)
    setManualError(null)
    dispatch({ type: 'SET_MANUAL_SEED', seed, version: manualVersion, worldType: manualWorldType })
  }
  ...
  return (
    <div className="flyout-panel">
      <div className="flyout-header">
        <span className="flyout-title">Seed</span>
      ...
      <div className="flyout-body">
        {state.seedData ? (
              ...
              <>
                <div className="flyout-row" style={{ marginBottom: 6 }}>
                  <select
                    ...
                    onChange={e => dispatch({ type: 'SET_VERSION', version: e.target.value as MCVersionKey })}
                ...
                {(['flat', 'single_biome', 'custom'] as WorldType[]).includes(state.worldType) ? (
                  ...
                  <div className="flyout-row" style={{ marginBottom: 6 }}>
                    <select
                      ...
                      onChange={e => dispatch({ type: 'SET_WORLD_TYPE', worldType: e.target.value as WorldType })}
            ...
            <div className="mp-world-toggle-row">
              <label className="overlay-toggle">
                <input type="checkbox" checked={state.showStructures}
                  onChange={() => dispatch({ type: 'TOGGLE_STRUCTURES' })} />
                <span className="overlay-label">Show Structures</span>
              </label>
            </div>
            <div className={`mp-struct-section${state.showStructures ? '' : ' mp-struct-section--disabled'}`}>
            <div className="mp-struct-header">
              ...
              <div className="mp-struct-header-btns">
                <button className="btn-sm" disabled={allEnabled}
                  onClick={() => dimStructures.forEach(s => !enabledStructures.has(s) && dispatch({ type: 'TOGGLE_STRUCTURE', structure: s }))}>
                  All
                </button>
                  ...
                  return (
                    <label key={s} className={`mp-struct-check${on ? ' on' : ''}`} title={cfg.summary}
                      ...
                      <input type="checkbox" checked={on}
                        onChange={() => dispatch({ type: 'TOGGLE_STRUCTURE', structure: s })} />
                        ...
                        {cfg.notableLoot && (
                          <label className="mp-variant-row mp-notable-row" title={cfg.notableLoot.summary}>
                            <input type="checkbox" checked={notableOn}
                              onChange={() => dispatch({ type: 'TOGGLE_NOTABLE_LOOT_ONLY', structure: s })} />
          ...
          <>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 6 }}>Enter a seed:</div>
            {seedEntry}
          </>
        )}
      </div>
